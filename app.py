...
# ------------------ SESSION STATE (เริ่มต้นว่างเปล่า) ------------------
if "products" not in st.session_state:
    st.session_state.products = {}  # ว่างเปล่า เพื่อให้ครูกรอกเอง
...
if menu == "📊 แดชบอร์ดภาพรวม":
    ...
    with c1:
        total_stock = sum(item["qty"] for item in st.session_state.products.values())
        st.markdown(f"""
        <div class="highlight-card">
            <h3>สินค้าคงคลังรวม</h3>
            <h1>{total_stock:,} ชิ้น</h1>
        </div>
        """, unsafe_allow_html=True)
...
# ------------------ 2. จัดการสต็อกสินค้า (เพิ่ม/แก้ไข/ลบ) ------------------
...
# ------------------ 4. บันทึกรายจ่ายอื่นๆ (บันทึก/ลบ) ------------------
...
# ------------------ 5. สรุปบัญชีประจำปี & ปันผล ------------------
    ...
    if net_profit > 0:
        ...
        st.warning("ไม่มีกำไรสุทธิในปีนี้ ไม่ต้องจัดสรรเงินปันผล")net_profit = total_rev - total_exp
